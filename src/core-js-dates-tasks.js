...
function getDayName(date) {
  ...
  return days.filter((_, index) => index === new Date(date).getDay()).join('');
}
...
function getNextFriday(date) {
  ...
  const daysToFriday = (12 - dayOfWeek) % 7 || 7;
  const nextFriday = new Date(date);
  nextFriday.setDate(dayOfMonth + daysToFriday);
  return nextFriday;
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  const startInMS = Date.parse(dateStart);
  const endInMS = Date.parse(dateEnd);

  return (endInMS - startInMS) / 1000 / 3600 / 24 + 1;
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  for (let i = 1; i <= days; i += 1) {
    ...
    if (day === 6 || day === 0) weekends.push(i);
  }
...
function getWeekNumberByDate(/* date */) {
  ...
  // const countDayFirstWeek = 7 - firstJanuaryDay.getDay();
  // const countDayTotal = (date - firstJanuaryDay) / 1000 / 3600 / 24;
  // const week = Math.ceil((countDayTotal - countDayFirstWeek) / 7) + 1;
  // return week;
